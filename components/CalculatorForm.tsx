...
export default function CalculatorForm() {
  ...
  async function handleCalculate() {
    ...
    setLoading(true);
    try {
      // 미국 주식은 티커 그대로, 한국 주식은 .KS 접미사 시도
      const ticker = market === "KR" ? `${stock.trim()}.KS` : stock.trim().toUpperCase();
      ...
      const grossAmount = dps * +quantity;
  ...
  const taxLabel = market === "KR"
    ? "한국 배당소득세 15.4% 자동 적용"
    : "미국 배당소득세 15% 자동 적용 (원천징수)";
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white rounded-2xl shadow-card p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={market === "KR" ? "예: 005930 (삼성전자 티커)" : "예: AAPL"}
          ...
          <p className="text-[11px] text-toss-sub px-1">
            {market === "KR" ? "한국 주식은 종목 코드(예: 005930)를 입력해 주세요." : "미국 주식은 티커(예: AAPL, MSFT)를 입력해 주세요."}
          </p>
        </div>
        ...
        {/* 세율 안내 */}
        <div className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl ${market === "KR" ? "bg-blue-50" : "bg-green-50"}`}>
        ...
        <button onClick={handleCalculate} disabled={loading}
          className="w-full bg-toss-blue hover:bg-toss-blueDark active:scale-[0.98] disabled:opacity-60
                     text-white font-bold text-[16px] py-4 rounded-2xl shadow-md transition-all duration-150 flex items-center justify-center gap-2">
          {loading
            ? <><Spinner /> 데이터 불러오는 중...</>
            : "배당 수익 계산하기"}
        </button>
